...
int main(int argc, char **argv)
{
    ...
    float pc = 0.7; /**< probability of correct transition */
    ...
    tx = 0;//rand()%sx;     // random target x
    ty = 39;//rand()%sy;     // random target y
    sim1->setTarget(tx, ty);
    printf("Target is set at the location [%d,%d]\n", tx, ty);
    
    id = 1;						// robot id
    r1 = new Robot(id, sim1);	// create your robot
    ix = 0;// rand()%sx;				// random initial x
    iy = 0;//rand()%sy;				// random initial y
